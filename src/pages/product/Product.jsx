...
import { applyMarkup, formatNumber } from '../../utils/priceUtils.js';
import './Product.css';
...
export default function Product() {
  ...
  const [lightbox, setLightbox]       = useState(false); // увеличенный просмотр
  const [toast, setToast]             = useState(null);  // { msg, type }

  useEffect(() => { loadProduct(); }, [id]);
  useEffect(() => {
    if (user && product) checkFavorite();
  }, [user, product]);
  ...
  const toggleFavorite = async () => {
    ...
    if (isFavorite) {
      await supabase.from('favorites').delete()
        .eq('user_id', user.id).eq('article', product.article);
      setIsFavorite(false);
      ...
      await supabase.from('favorites').insert({
        ...
        name:      product.name,
        price:     applyMarkup(getProductPrice()),
  ...
  const handleAddToCart = () => {
    ...
    const cartItem = {
      ...
      title:   product.name,
      name:    product.name,
      price:   price, // applyMarkup применяется в CartContext
  ...
  const priceWithMarkup = applyMarkup(currentPrice);
        ...
        <div className="product-content">
          ...
          <div className="product-details">
            ...
            <div className="product-price-block">
              <span className="product-price">{formatNumber(priceWithMarkup)} ₸</span>
              {product.price1 && product.price1 !== 1 && product.price2 && (
                <span className="product-price-old">{formatNumber(applyMarkup(product.price1))} ₸</span>
              )}
            </div>
