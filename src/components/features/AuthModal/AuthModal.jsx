...
export default function AuthModal({ onClose }) {
  ...
  const [step, setStep]         = useState(1);   // для телефона: 1=ввод, 2=OTP
  ...
  const handlePhoneSend = async (e) => {
    ...
    if (error) { setError(error.message); return; }
    setStep(2);
    setSuccess('Код отправлен на ' + formatted);
  };
      ...
      <div className="auth-modal">
        ...
        <div className="auth-tabs">
          <button
            ...
            onClick={() => { setTab('login'); setStep(1); clearMessages(); }}
          >Вход</button>
          <button
            className={`auth-tab ${tab === 'register' ? 'active' : ''}`}
            onClick={() => { setTab('register'); setStep(1); clearMessages(); }}
          >Регистрация</button>
          <button
            className={`auth-tab ${tab === 'phone' ? 'active' : ''}`}
            onClick={() => { setTab('phone'); setStep(1); clearMessages(); }}
          >По телефону</button>
        </div>
        ...
        {tab === 'register' && (
          <form className="auth-form" onSubmit={handleRegister}>
            <div className="auth-field">
              <label>Имя</label>
              <input
                type="text" value={fullName} required autoFocus
        ...
        {/* ── PHONE ── */}
        {tab === 'phone' && step === 1 && (
          <form className="auth-form" onSubmit={handlePhoneSend}>
            <div className="auth-field">
              <label>Номер телефона</label>
              <input
                type="tel" value={phone} required autoFocus
                onChange={e => setPhone(e.target.value)}
                placeholder="+77001234567"
              />
            </div>
            <p className="auth-hint">Отправим SMS с кодом подтверждения</p>
            <button type="submit" className="auth-btn-primary" disabled={loading}>
        ...
        {tab === 'phone' && step === 2 && (
          <form className="auth-form" onSubmit={handleOtpVerify}>
            ...
            <button type="button" className="auth-btn-ghost" onClick={() => { setStep(1); clearMessages(); }}>
              ← Изменить номер
            </button>
          </form>
        )}
