...
// Rate limiting
let lastApiCall = 0;
const MIN_INTERVAL = 6000; // 6 секунд!
...
async function waitForRateLimit() {
  ...
  if (timeSinceLastCall < MIN_INTERVAL) {
    ...
    console.log(`⏳ Ждем ${Math.round(waitTime/1000)}с...`);
...
app.get('/api/products', async (req, res) => {
  try {
    ...
    console.error('❌ Ошибка products:', error.message);
    res.status(500).json({ 
      error: error.message,
      elements: [],
...
app.get('/api/categories', async (req, res) => {
  try {
    ...
    if (!categories) {
      ...
      // Категории возвращают просто массив
...
app.get('/api/filters', async (req, res) => {
  try {
    ...
    try {
      ...
      // ВАЖНО! Бренды возвращают {status: true, data: [...]}
